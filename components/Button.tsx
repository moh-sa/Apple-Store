...
const Button = ({ title, width, loading, padding, noIcon, onClick }: Props) => {
  return (
    <button
      className={`ease group relative z-30 box-border inline-flex ${
        width ? width : "w-auto"
      } ${padding} bgGradiant cursor-pointer items-center justify-center overflow-hidden rounded bg-indigo-600 px-8 py-3 font-bold text-white transition-all duration-300 focus:outline-none`}
      onClick={onClick}
    >
